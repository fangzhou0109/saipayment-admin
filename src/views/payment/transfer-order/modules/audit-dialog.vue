<template>
  <el-drawer
    v-model="visible"
    :title="readonly ? '代付订单详情' : '代付订单审核'"
    size="680px"
    @open="handleOpen"
  >
    <el-descriptions v-loading="loading" :column="1" border>
      <el-descriptions-item label="平台代付单号">{{ detail.withdraw_no }}</el-descriptions-item>
      <el-descriptions-item label="商户代付单号">{{
        detail.out_biz_no || '-'
      }}</el-descriptions-item>
      <el-descriptions-item label="商户号">{{ detail.mch_id }}</el-descriptions-item>
      <el-descriptions-item label="代付金额">{{ detail.amount }} 元</el-descriptions-item>
      ...
      <el-descriptions-item label="上游代付单号">{{
        detail.transfer_no || '-'
      }}</el-descriptions-item>
      <el-descriptions-item label="下游通知地址">{{
        detail.notify_url || '-'
      }}</el-descriptions-item>
      ...
      <el-descriptions-item label="进单时间">{{ detail.create_time || '-' }}</el-descriptions-item>
    </el-descriptions>
    ...
    <el-card class="bank-card-panel" shadow="never">
      <template #header>
        <div class="bank-card-panel__title">
          ...
          <span>收款银行卡</span>
          <el-tag v-if="detail.account_no" size="small" type="info">进单快照</el-tag>
        </div>
      </template>
    ...
    <template v-if="!readonly && detail.status === 0">
      ...
      <el-form ref="auditFormRef" :model="auditForm" :rules="auditRules" label-width="100px">
        ...
        <el-form-item v-if="auditForm.action === 'disburse'" label="代付渠道" prop="channel_id">
          <el-select
        ...
        <el-alert
          ...
          :closable="false"
          show-icon
          title="提交后将调用上游代付接口，最终成功/失败由代付回调决定，并异步通知下游。"
        />
      </el-form>
    </template>
